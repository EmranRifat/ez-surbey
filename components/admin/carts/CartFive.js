...
function CartFive({ title, amount, groth, memberImg, totalEarnImg, transactionCount_state, transactionCount_state_loading ,timeFrame,  transactionCount_state_fetching }) {
  ...
  return (
    <div className="rounded-lg p-5 shadow-lg bg-gradient-to-b from-[#C4A7F4] to-[#EEFFF5] dark:bg-darkblack-600">
          ...
          <div className=" ">
            <div className="text-3xl font-bold leading-[48px] ml-4 text-[#223C55] dark:text-white  flex">
              {transactionCount_state_loading &&
                ...
                // transactionCount_state?.data?.total_unpaid_amount?.toFixed(2)

                !isNaN(parseFloat(transactionCount_state?.data?.total_unpaid_amount)) ? 
                parseFloat(transactionCount_state?.data?.total_unpaid_amount).toFixed(2) : 
                '0.00'
              )}  <svg
